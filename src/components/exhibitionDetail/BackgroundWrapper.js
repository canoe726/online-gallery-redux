...
const BackgroundWrapper = ({ slideIdx, data, changeSlideIdx }) => {
  ...
  return (
    <div
      ...
      onWheelCapture={wheelChangeExhibition}
      onTouchMoveCapture={wheelChangeExhibition}
  ...
  function wheelChangeExhibition (e) {
    const wheelDir = e.nativeEvent.wheelDelta;
    if (!throttler) {
      throttler = setTimeout(() => {
        let isScrollUp = false;
        // console.log(e.deltaY);
        if (wheelDir < 0) isScrollUp = true;
        ...
        if (!isScrollUp) {
          ...
          if (slideIdxTemp < 0) dispatch(changeSlideIdx(length - 1));
          else dispatch(changeSlideIdx(slideIdxTemp));
        // 다음 전시
        } else {
          ...
          const slideIdxTemp = slideIdx + 1;
          if (slideIdxTemp >= length) dispatch(changeSlideIdx(0));
          else dispatch(changeSlideIdx(slideIdxTemp));
        }
        throttler = null;
      }, 300);
